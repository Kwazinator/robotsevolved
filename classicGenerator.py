        ...
        while (not (easyflag and mediumflag and hardflag and exteremlyhardflag)):
            ...
            if (moves >= 23 and not godflag):
                ...
                with app.app_context():
                    GeneratorService().insert_puzzle('classic', 'GodTeir', generator.formatsolutiondata(solution), 'abcdefg', moves, json.dumps(solutiondata))
                    godflag = True
                    print('found turkutier puzzle of ' + str(moves) + ' moves')
            elif (17 < moves < 23 and not exteremlyhardflag):
                app = flaskr.create_app()
                with app.app_context():
                    GeneratorService().insert_puzzle('classic', 'Exteremly Hard', generator.formatsolutiondata(solution), 'abcdefg', moves, json.dumps(solutiondata))
                    exteremlyhardflag = True
                    print('found exteremlyhard puzzle of ' + str(moves) + ' moves')
            elif (13 < moves <= 17 and not hardflag):
                app = flaskr.create_app()
                with app.app_context():
                    GeneratorService().insert_puzzle('classic', 'hard', generator.formatsolutiondata(solution), 'abcdefg', moves, json.dumps(solutiondata))
                    hardflag = True
                    print('found hard puzzle of ' + str(moves) + ' moves')
            elif (8 < moves <= 13 and not mediumflag):
                app = flaskr.create_app()
                with app.app_context():
                    GeneratorService().insert_puzzle('classic', 'medium', generator.formatsolutiondata(solution), 'abcdefg', moves, json.dumps(solutiondata))
                    mediumflag = True
                ...
                with app.app_context():
                    ...
                    GeneratorService().insert_puzzle('classic', 'easy', generator.formatsolutiondata(solution), 'abcdefg', moves, json.dumps(solutiondata))